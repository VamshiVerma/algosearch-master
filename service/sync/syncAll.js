...
async function getSyncedRound() {
	...
	console.log(`round what we got }: `);

	// Query for view and retrieve the latest block
	await nano.db.use('blocks').view('latest', 'latest', {include_docs: true, descending: true, limit: 10}).then((body) => {
		console.log(`body what we got `);

		round = body.rows[0].doc.round; // Get the round number from the latest block
		console.log(`round what we got ${round}: `);
